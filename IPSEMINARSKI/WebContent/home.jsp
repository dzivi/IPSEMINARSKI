	...
	<form action="Post" method="post">
		...
		<p align="center"><input type="submit" value='Post' class='signupfield'></p>
					...
					User user = new User();
								...
								postovi=post.getPostsOfMyFriends(logged.getUserID());
								if(postovi!=null && postovi.size()>0){
									for(Post pom : postovi){
										user = userDAO.getUserbyID(pom.getUserID());
										%>
										<%=Posts(pom, user.getUserID(), user.getProfilePicture(), user.getFirstName()+" "+user.getLastName(), logged.getUserID()) %>
										<%
									}
								}
