		...
		<div id="contentcontainer">
			<div id="content">
				...
				<div id="centercontent">
					...
					<%
					if(fd.weAreFriends(logged.getUserID(), userProfile.getUserID())!=0 || logged.getUserID()==userProfile.getUserID() || logged.getLevelID()==2){
					if(logged.getUserID()==userProfile.getUserID()){%>
							
							<form action="Post" method="post">
								<div class='singlecomment'>
								<textarea placeholder='...insert text...' name='Text'></textarea><br><br> 
								<input type='text' placeholder='...insert URL address here...' name='Link' size="52"><br><br>
								<input type="hidden" name="UserID" value="<%=logged.getUserID()%>">
								<p align="center"><input type="submit" value='Post' class='signupfield'></p>
								</div>
							</form>
							...
							PostDAO post = new PostDAO();
								ArrayList <Post> postovi = new ArrayList<Post>();
								postovi=post.getPostsByID(userProfile.getUserID());
								if(postovi!=null && postovi.size()>0){
									for(Post pom : postovi){
										%>
										<%=Posts(pom, userProfile.getUserID(), userProfile.getProfilePicture(), userProfile.getFirstName()+" "+userProfile.getLastName(), logged.getUserID()) %>
										<%
									}
								} else {
									%>
					...
					} else if(fd.existFriendship(logged.getUserID(), userProfile.getUserID())!=0){
						%>
						<%=userProfile.getFirstName() %> has already been added, or you have a request from <%=userProfile.getFirstName() %>
						<%
					} else {
						
						%>
						<form action="SendRequest" method="post">
						<input type="hidden" name="UserReqID" value="<%=logged.getUserID()%>">
						<input type="hidden" name="UserRespID" value="<%=userProfile.getUserID()%>">
						<input type="submit" value="Add friend" id="signupbutton">
						</form>
						<%
					}
